    ...
    while (1) {
        ...
        if (find_unprocessed_block(shm, sizeof(int), &block, &offset)) {
            ...
            if (get_block_stats(block, &min_val, &max_val)) {
                ...
                printf("[ПОТРЕБИТЕЛЬ #%d] Блок: мин=%d, макс=%d, чисел=%d, offset=%d\n", 
            ...
            printf("[ПОТРЕБИТЕЛЬ #%d] Обработано блоков: %d, засыпает на %d сек\n", 
            ...
            if (idle_cycles >= 1) {
                printf("[ПОТРЕБИТЕЛЬ #%d] Нет данных, ожидание... (цикл %d)\n", 
                       id, idle_cycles);
            }
            ...
            if (idle_cycles >= max_idle_cycles) {
                ...
                if (prod_finished && !final_check) {
                    printf("[ПОТРЕБИТЕЛЬ #%d] Завершаю работу (max idle cycles)\n", id);
                    break;
                }
                idle_cycles = 0;
            }
